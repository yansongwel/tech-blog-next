    ...
    <div>
      ...
      {loading ? (
        <div className="glass rounded-xl overflow-hidden animate-pulse">
          <div className="border-b border-border bg-surface/50 px-4 py-3 flex gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-3 bg-surface rounded w-20" />)}</div>
          {[...Array(6)].map((_, i) => <div key={i} className="flex items-center gap-4 px-4 py-4 border-b border-border/30"><div className="h-4 bg-surface rounded flex-1" /><div className="h-4 bg-surface rounded w-20" /><div className="h-4 bg-surface rounded w-16" /></div>)}
        </div>
